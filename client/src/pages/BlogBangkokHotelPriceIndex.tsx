...
const priceTiers = [
  { id: "bangkok-budget-tier", label: "Hostels and budget", range: "$8–$35/night", description: "The published budget range begins at $8–$16 in Khao San and Old City and reaches $18–$35 along the Riverside; transit access is part of the value trade-off." },
  { id: "bangkok-midrange-tier", label: "Three- and four-star mid-range", range: "$35–$150/night", description: "Mid-range benchmarks span $35–$70 in Khao San and Old City through $75–$150 on the Riverside, with Sukhumvit, Silom, and Siam in between." },
  ...
  return (
    <div className="min-h-screen bg-[#FBF8F1] pb-20 md:pb-0">
      ...
      <main className="container max-w-6xl px-4 py-12 md:py-16"><ArticleBreadcrumbs currentLabel="Bangkok hotel price index" /><a href="/blog" className="mb-8 inline-flex items-center gap-2 text-sm font-semibold text-[#0077B6] hover:text-[#005c91]"><ArrowLeft className="h-4 w-4" aria-hidden="true" />Back to Blog</a>
        ...
        <section className="mt-14" aria-labelledby="bangkok-price-tiers-title"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#0077B6]">Jump by tier</p><h2 id="bangkok-price-tiers-title" className="mt-3 font-playfair text-3xl font-bold text-[#0D1B2A] md:text-4xl">Bangkok price tiers at a glance</h2><div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{priceTiers.map((tier) => <article id={tier.id} key={tier.id} className="scroll-mt-28 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0077B6]">{tier.label}</p><h3 className="mt-2 font-playfair text-2xl font-bold text-[#0D1B2A]">{tier.range}</h3><p className="mt-3 text-sm leading-relaxed text-slate-700">{tier.description}</p></article>)}</div></section>
      ...
      </main><section className="container max-w-6xl px-4 pb-16 md:pb-20"><RelatedPriceIndexArticles destination="bangkok" /></section><Footer /><MobileBottomNav /></div>
  );
}
