...
export default function BangkokArrivalChecklist() {
  ...
  useEffect(() => {
    ...
    const savedTransferReference = window.localStorage.getItem("tsw-bangkok-transfer-reference");
    if (savedTransferReference) setTransferReference(savedTransferReference);
  ...
  useEffect(() => {
    if (transferReference.trim()) window.localStorage.setItem("tsw-bangkok-transfer-reference", transferReference.trim());
    else window.localStorage.removeItem("tsw-bangkok-transfer-reference");
  }, [transferReference]);
  ...
  const clearLocalDetails = () => {
    ...
    window.localStorage.removeItem("tsw-bangkok-hotel-confirmation");
    window.localStorage.removeItem("tsw-bangkok-transfer-reference");
    setShareStatus("Hotel and transfer references cleared from this browser.");
  };
  const digitalChecklistUrl = "https://thestayandwander.com/blog/bangkok-hotel-price-index-2026#bangkok-arrival-checklist-heading";
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&format=svg&data=${encodeURIComponent(digitalChecklistUrl)}`;
