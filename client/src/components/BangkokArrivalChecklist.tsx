...
export default function BangkokArrivalChecklist() {
  ...
  const googleCalendarUrl = (kind: "pickup" | "checkin") => {
    const date = kind === "pickup" ? pickupDate : checkInDate;
    const time = kind === "pickup" ? pickupTime : checkInTime;
    if (!date) return "";
    const start = new Date(`${date}T${time}:00`);
    ...
    const baseTitle = kind === "pickup" ? "Bangkok airport pickup" : "Bangkok hotel check-in";
    const title = tripTitle.trim() ? `${tripTitle.trim()} — ${baseTitle}` : baseTitle;
    const details = kind === "pickup" ? "Confirm transfer vehicle, luggage, and airport pickup point." : "Confirm hotel address, reservation, and arrival details.";
    return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${stamp(start)}/${stamp(end)}&details=${encodeURIComponent(details)}`;
  };
  ...
  return (
    <section className="mt-12 rounded-3xl border border-[#cfe4ee] bg-[#eef8fb] p-6 md:p-8" aria-labelledby="bangkok-arrival-checklist-heading">
      ...
      <div className="mt-7 rounded-2xl border border-white bg-white p-5"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0077B6]">Calendar reminders</p><h3 className="mt-2 font-playfair text-xl font-bold text-[#0D1B2A]">Add pickup and check-in to your calendar</h3><label className="mt-4 block max-w-xl text-sm font-semibold text-[#0D1B2A]">Optional trip title<input type="text" value={tripTitle} onChange={(event) => setTripTitle(event.target.value)} placeholder="e.g. Tanih’s Bangkok getaway" className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 font-normal text-slate-800" /></label><p className="mt-2 text-xs text-slate-500">The title is added before each reminder name in both calendar formats.</p><div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-xl bg-[#eef8fb] p-4"><p className="font-semibold text-[#0D1B2A]">Airport pickup</p><div className="mt-3 grid grid-cols-2 gap-3"><label className="text-sm text-slate-700">Date<input type="date" value={pickupDate} onChange={(event) => setPickupDate(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">Time<input type="time" value={pickupTime} onChange={(event) => setPickupTime(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" /></label></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => downloadCalendarReminder("pickup")} className="inline-flex items-center gap-2 rounded-full bg-[#0077B6] px-4 py-2 text-sm font-semibold text-white"><CalendarPlus className="h-4 w-4" aria-hidden="true" />Download Reminder</button>{googleCalendarUrl("pickup") ? <a href={googleCalendarUrl("pickup")} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#0077B6] bg-white px-4 py-2 text-sm font-semibold text-[#0077B6]">Open Google Calendar</a> : <span className="self-center text-xs text-slate-500">Choose a date to enable Google Calendar.</span>}</div></div><div className="rounded-xl bg-[#eef8fb] p-4"><p className="font-semibold text-[#0D1B2A]">Hotel check-in</p><div className="mt-3 grid grid-cols-2 gap-3"><label className="text-sm text-slate-700">Date<input type="date" value={checkInDate} onChange={(event) => setCheckInDate(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">Time<input type="time" value={checkInTime} onChange={(event) => setCheckInTime(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" /></label></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => downloadCalendarReminder("checkin")} className="inline-flex items-center gap-2 rounded-full bg-[#0077B6] px-4 py-2 text-sm font-semibold text-white"><CalendarPlus className="h-4 w-4" aria-hidden="true" />Download Reminder</button>{googleCalendarUrl("checkin") ? <a href={googleCalendarUrl("checkin")} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#0077B6] bg-white px-4 py-2 text-sm font-semibold text-[#0077B6]">Open Google Calendar</a> : <span className="self-center text-xs text-slate-500">Choose a date to enable Google Calendar.</span>}</div></div></div></div>
