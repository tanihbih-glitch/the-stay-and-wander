...
const vehicleOptions = [
  { id: "sedan", label: "Sedan", capacity: "1–3 travelers", luggage: "Up to 2 medium suitcases or 3 cabin bags", maxTravelers: 3, maxCheckedBags: 2, maxCabinBags: 3, multiplier: 1 },
  { id: "minivan", label: "Minivan", capacity: "4–6 travelers", luggage: "About 4 medium suitcases plus 6 cabin bags", maxTravelers: 6, maxCheckedBags: 4, maxCabinBags: 6, multiplier: 1.55 },
  { id: "bus", label: "Minibus / Bus", capacity: "7–12 travelers", luggage: "About 8 medium suitcases plus 12 cabin bags", maxTravelers: 12, maxCheckedBags: 8, maxCabinBags: 12, multiplier: 2.2 },
] as const;
...
export default function BangkokHotelTaxCalculator() {
  ...
  const [checkedBags, setCheckedBags] = useState(2);
  ...
  const [vehicle, setVehicle] = useState<VehicleId | "auto">("auto");
  const [transferCurrency, setTransferCurrency] = useState<TransferCurrency>("USD");
  const recommendedVehicle = vehicleOptions.find((option) => travelers <= option.maxTravelers && checkedBags <= option.maxCheckedBags && cabinBags <= option.maxCabinBags) ?? vehicleOptions[2];
  const selectedVehicle = vehicle === "auto" ? recommendedVehicle : vehicleOptions.find((option) => option.id === vehicle) ?? recommendedVehicle;
