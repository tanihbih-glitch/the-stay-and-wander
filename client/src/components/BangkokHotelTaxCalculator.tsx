...
export default function BangkokHotelTaxCalculator() {
  ...
  const summary = `Bangkok stay budget: ${money.format(nightlyRate)} per night × ${nights} night${nights === 1 ? "" : "s"}. Room subtotal ${money.format(result.subtotal)}, estimated ${surchargeRate}% surcharge ${money.format(result.surcharge)}, estimated total ${money.format(result.total)}. Planning estimate from The Stay & Wander.`;
  const shareBudget = async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: "Bangkok stay budget", text: summary, url: "https://thestayandwander.com/blog/bangkok-hotel-price-index-2026" });
  ...
  return (
    <section className="mt-14 rounded-3xl border border-[#cfe4ee] bg-[#eef8fb] p-6 md:p-8" aria-labelledby="bangkok-tax-calculator-heading">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3"><button type="button" onClick={shareBudget} className="inline-flex items-center gap-2 rounded-full bg-[#0077B6] px-5 py-3 text-sm font-semibold text-white hover:bg-[#005c91]"><Share2 className="h-4 w-4" aria-hidden="true" />Share My Bangkok Stay Cost</button>{shareStatus && <p className="inline-flex items-center gap-2 text-sm text-slate-700"><Check className="h-4 w-4 text-emerald-600" aria-hidden="true" />{shareStatus}</p>}</div>
    </section>
  );
}
