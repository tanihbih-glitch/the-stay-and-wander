...
- [ ] Diagnose and correct the OAuth callback verification failure observed after the user-approved read-only consent, then restart authorization safely. The callback now issues `SameSite=None; Secure` state cookies on HTTPS to survive Google’s cross-site authorization return; publish and retry consent.
