...
- [ ] Configure a read-only Google Search Console connection for automated monthly CTR monitoring of the Bali, Bangkok, Tokyo, and Seoul where-to-stay guides.
...
- [ ] Validate the data access and schedule, document the report cadence and monitored URLs, and publish the completed FAQ/itinerary/monitoring enhancement checkpoint. The monitoring documentation is complete; OAuth connection and schedule activation remain pending credentials.
...
- [ ] Complete user authorization to obtain a refresh token, validate read-only access to the `sc-domain:thestayandwander.com` Search Console property, and preserve no more token data than required. Local endpoint verification and the full 96-test suite pass; user consent remains pending after publication.
...
- [ ] Use the user-selected Google account for the read-only Search Console consent flow, inspect the requested permission, and obtain final confirmation before approving access. Google currently blocks the selected account because the OAuth app is in testing mode and the account has not been added as an approved test user.
...
- [ ] Inspect the Search Console API-returned site URLs during callback validation and match the user-verified `sc-domain:thestayandwander.com` and `https://thestayandwander.com/` properties exactly before the next authorization attempt. The callback now normalizes and records only relevant returned property URLs, prefers the domain property, and accepts the exact URL-prefix fallback; focused tests and TypeScript validation pass. Publish before retrying authorization.
