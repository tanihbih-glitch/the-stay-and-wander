...
- [x] Save and publish the enhanced city planner controls in checkpoint `d71f8ff1`, then verify the test-ready routes. After propagation, cache-busted custom-domain browser flows passed across Bangkok, Seoul, and Tokyo for built-in/custom template previews and application, persisted Violet template tags, archive/show archive/restore/delete lifecycle, and all previously covered matcher controls.
...
- [x] Publish the approved eligibility-based update in checkpoint `906cbe33`.
...
- [x] Publish the enhanced Seoul and Tokyo comparison tools in checkpoint `906cbe33`.
- [x] Defer distinct Seoul and Tokyo neighborhood-level numerical rate ranges until a credible source or user-supplied survey provides them; the user approved the transparent eligibility-based scope.
- [x] Verify the propagated published Seoul and Tokyo routes show the approved City-Tier label, localized-survey notice, individual-area notes, and fragment-only sharing behavior. Fresh managed-domain flows passed for Bangkok, Seoul, and Tokyo, including shared-list load with private notes excluded from the URL.
...
- [x] Audit the 21 destination guides and identify long-form non-price guides with stable section anchors for table-of-contents navigation.
- [x] Add a browser-local Was this guide helpful? thumbs-up/down widget to the bottom of every destination guide without collecting visitor identity or personal data.
...
- [x] Audit the four priority guides’ current titles/descriptions, supplied Search Console baseline, homepage and key-guide branding, Organization/WebSite schema, and UAE guide internal discovery/content depth. The Bali and Bangkok legacy paths are 301 redirects, so their canonical destination guides are the actionable snippet targets; brand-query results also reveal similarly named travel and media entities.
- [x] Apply unique 50–60 character titles and 140–155 character meta descriptions to the four priority routes without modifying H1s, URLs, body copy, or existing ranking-relevant structure.
- [x] Document and exclude low-intent AI-assistant-style multilingual sustainability, loyalty, and accessibility query patterns from future keyword and content-brief targeting.
- [x] Address only verified UAE guide discoverability gaps through appropriate internal linking, on-page metadata/schema, or evidence-backed on-page guidance; document external backlink needs separately without fabricating links. Added a reciprocal contextual link from the Dubai/Abu Dhabi guide; did not invent rate evidence or backlinks.
- [x] Add regression coverage and validate crawler-visible metadata, canonical URLs, brand Organization/WebSite schema, homepage and booking metadata preservation, and internal discovery. Focused checks and full tests pass (35 files / 183 tests), production build passes, local raw SSR confirms four exact snippets plus Organization/WebSite JSON-LD, and responsive visual review preserves every H1.
- [x] Schedule and document a Search Console follow-up comparison in 3–4 weeks against the July 13–September 6, 2026 baseline, then publish and verify the completed CTR work. Checkpoint `35a6fb85` is live; the enabled project task `JXTFqNB6NnfvtLrgKHXNgM` runs next on October 1, 2026 at 09:00 UTC and compares CTR/position deltas for the four priority canonical guides. Cache-busted custom-domain HTML confirms all four snippets plus Organization/WebSite JSON-LD; legacy Bali and Bangkok paths retain redirects.

- [x] Capture a before-and-after record of the four current live snippets and verify the exact price or cost figures supported by each article before revising any CTR copy. The live audit is saved in `docs/ctr-snippet-audit-2026-09-12.md`; Bali and Bangkok legacy paths resolve with 301 redirects to their canonical first-timer guides.
- [x] Refine the four priority metadata entries with unique 50–60 character titles and 140–155 character descriptions grounded only in their verified on-page figures, while retaining H1s, URLs, content structure, homepage metadata, and booking metadata.
- [x] Reconfirm the documented exclusion of low-intent multilingual AI-assistant prompts, current Organization/WebSite branding signals, UAE internal-link/depth findings, and the scheduled CTR comparison. The enabled task `JXTFqNB6NnfvtLrgKHXNgM` now runs October 8, 2026 at 09:00 UTC—25 days after the September 13 review date.
- [x] Add regression coverage and validate live crawler metadata, redirects, brand schema, and protected-page metadata before publishing the refined CTR release. Focused checks pass (7 files / 47 tests), full tests pass (35 files / 183 tests), production build passes, and local crawler HTML confirms all four exact snippets plus preserved homepage/booking titles and legacy redirects.
- [x] Publish and verify the refined metadata release, including a live before-and-after snippet record. Checkpoint `8c79c16b` is live; cache-busted custom-domain HTML confirms each refined title, description, Organization/WebSite markup, retained legacy 301 redirects, and preserved homepage/booking snippets. The before-and-after record remains in `docs/ctr-snippet-audit-2026-09-12.md`.

- [x] Research first-party sources for UAE extended-stay brand formats, amenities, loyalty, sustainability frameworks, and strong UAE-specific options; save the source record and clearly separate unverified rate claims. The audit record covers Residence Inn, Executive Apartments context, Adagio, Staybridge, Hilton global-format boundaries, and corporate sustainability frameworks.
- [x] Audit reusable price-index interaction architecture, routing, structured-data, sitemap, legacy redirects, internal-discovery registries, and existing Search Console monitoring before designing the new hub.
- [x] Create `/blog/uae-extended-stay-hotels-2026` as a cited comprehensive extended-stay comparison hub with a six-factor comparison model, traveler-fit sections, unique SEO metadata, visible FAQ content, and crawler-visible schema.
- [x] Build a deterministic, accessibility-conscious “Find Your Best Extended-Stay Hotel” selector that ranks two or three source-supported options for the selected traveler type and priority factor without collecting visitor data.
- [x] Add relevant internal discovery links, route/sitemap metadata, and a distinct UAE hub Search Console monitoring target focused on position and citation presence rather than CTR alone. The original October CTR task remains untouched; the new handler uses separate storage and a manual citation-review prompt.
- [ ] After the new canonical hub is confirmed indexed, add the safe 301 migration from `/blog/uae-extended-stay-sustainability-2026` and update residual internal links to the new URL.
...
- [ ] Publish the UAE extended-stay comparison hub, create the independent first-business-day monitoring job, and verify cache-busted custom-domain routes. The legacy route must remain live and unredirected until Search Console confirms indexing.
