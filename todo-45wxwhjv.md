...
- [ ] Add reusable schema and UI regression coverage, verify each page’s server-rendered JSON-LD matches visible FAQ content, and validate the six published URLs with Google’s Rich Results Test. Local tests confirm four FAQ pairs per article and one SSR FAQPage script per target route; Google validation remains pending publication.
- [ ] Save a checkpoint and confirm the live FAQ sections and structured-data deployment.
