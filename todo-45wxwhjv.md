...
- [ ] Secure the supplied Google OAuth client credentials and implement the registered `https://thestayandwander.com/api/oauth/google-search-console/callback` route with a strict read-only Search Console scope. The callback, CSRF state cookie, AES-GCM refresh-token encryption, database storage, and strict `webmasters.readonly` request are implemented and locally verified; publish the authorization route before user consent.
